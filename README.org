...
prompt2 requires *iniparser 4.2 or newer*. prompt2 will still build
against 4.1.x (which some Debian/Ubuntu releases still ship), but 4.1.x
handles backslashes differently, so a config written for 4.2 shows up
with stray backslashes in your prompt. Check with
=dpkg-query -W libiniparser-dev=; if it's too old, build iniparser from
[[https://gitlab.com/iniparser/iniparser][source]] first.

| Note on the build instructions                                        |
|-----------------------------------------------------------------------|
| If the instructions below don't work for you, I advise that you check the  github action for your operating system([[.github/workflows/build-and-test.yml#L12][Debian/Ubuntu]], [[.github/workflows/build-and-test.yml#L52.yml][MacOS]]), since this should be up-to-date. |
...
If you'd rather not edit the INI file directly, there's a browser-based
visual editor in the [[prompt2-config-ui/][prompt2-config-ui/]] directory. It gives you a live
preview of your prompt, a searchable colour picker, and a side-by-side view
of your original and updated config. See [[prompt2-config-ui/README.md][its README]] for how to start it.
